...
class DualHexagonChart extends StatelessWidget {
  final Map<String, double> selfPerceptionSkills;
  final Map<String, double> averageOpinionSkills;
  final int numberOfOpinions;
  final double size;

  const DualHexagonChart({
    Key? key,
    required this.selfPerceptionSkills,
    required this.averageOpinionSkills,
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        CustomPaint(
          size: Size(size, size),
          painter: DualHexagonPainter(
            selfPerceptionSkills,
            averageOpinionSkills,
          ),
        ),
...
class DualHexagonPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Colores
    final blueColor = const Color(0xFF6366F1); // Azul para autopercepción
    final redColor = const Color(0xFFEF4444); // Rojo para opiniones
    ...
    // Dibujar hexágono de opiniones promedio (atrás)
    _drawSkillsHexagon(canvas, center, radius, redColor, averageOpinionSkills);

    // Dibujar hexágono de autopercepción (adelante)
    _drawSkillsHexagon(canvas, center, radius, blueColor, selfPerceptionSkills);
